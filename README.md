...
> 公开版本是 **Mock 演示**，无需 Key，不调用真实模型。真实衣物识别和点评有小样本定向通过记录；真实三套搭配生成仍未通过稳定性验收。个人主导产品设计与验证，使用 AI 辅助代码实现和排障，尚非商用产品。
...
3. “我的” → “我的方案”，查看保存结果。
4. 衣橱体验图片选择、标签修正与入库。**Mock 标签不代表真实识别了上传图片。**

## 产品问题与范围

目标假设：大学生在上课、面试、运动等场景切换时，需要从已有衣服中更快做出搭配决定。先验证已有衣橱的组合与使用，而不是增加购物推荐。关于决策耗时、闲置率和用户收益的假设尚待访谈与实测。
...
| 灵感社区 | Seed 信息流、组合点评与本地帖子，不是多人实时社区 |
...
上述是定向用例，不是总体准确率。[查看原始记录](docs/evidence/README.md)。既有 175 项工程回归与随后 8 项增量回归有记录，不能等同真实模型验收，也不宣称本次全部重跑。

## 界面预览

390×844 浏览器视口；Seed / Mock 数据，不是手机硬件测试。
...
React 18、TypeScript、Vite、Tailwind CSS、Zustand、React Router。四家 provider 调用路径已实现，不代表四家真实服务全部联调通过。Key 仅由本机 Node 代理读取，公开版没有服务端。
...
打开 http://localhost:5173 。无 Key 默认 Mock。需要真实联调时复制 `.env.example` 为 `.env`，只在自己机器设置 `AI_KEY` 与 provider；**不要提交 .env，不要将 Key 设为 VITE_ 变量**。
...
真实搭配的超时和输出质量、多样衣物识别、手机硬件兼容与用户访谈仍待补齐。天气、社区热度、估值与节省时间均是演示或估算；虚拟试穿、真实社交、账号同步和离线服务未实现。图片以 base64 留在浏览器存储，大图可能超出容量；请勿上传私人照片。

[简历与面试讲解材料](docs/portfolio/简历与投递.md)。公开演示不等于正式商用，不宣称无证据的增长、节时或准确率。
